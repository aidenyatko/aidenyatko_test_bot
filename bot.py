...
async def process_help_command(message: Message):
    await message.answer(f'Правила игры:\n\nЯ загадываю число от 1 до 100, '
                         ...
                         f"Чтобы начать игру напиши 'Да', 'Давай', 'Сыграем', 'Игра', 'Играть', 'Хочу играть'. "
                         "После каждой попытке я буду тебе говорить 'больше' или 'меньше'")
...
async def process_numbers_answer(message: Message):
    if users[message.from_user.id]['in_game']:
        if int(message.text) == users[message.from_user.id]['secret_number']:
            ...
            await message.answer(f"Мое число меньше, у тебя осталось {users[message.from_user.id]['attempts']}")
        elif int(message.text) < users[message.from_user.id]['secret_number']:
            users[message.from_user.id]['attempts'] -= 1
            await message.answer(f"Мое число больше, у тебя осталось {users[message.from_user.id]['attempts']}")
